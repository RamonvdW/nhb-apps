...
TEMPLATE_LIJST_BEHEERDERS_COMPETITIE = 'functie/lijst-beheerders-competitie.dtl'


class BeheerdersCompetitieView(UserPassesTestMixin, ListView):
    ...
    template_name = TEMPLATE_LIJST_BEHEERDERS_COMPETITIE
    ...
    def test_func(self):
        ...
        return self.rol_nu in (Rol.ROL_BB, Rol.ROL_MWZ, Rol.ROL_SUP,
    ...
    def _sorteer_functies(objs):
        ...
        """
        sort_level = {'BKO': 1, 'RKO': 2, 'RCL': 3, 'MWZ': 10, 'CS': 11 , 'SUP': 12}
        tup2obj = dict()
        sort_me = list()
        for obj in objs:
            if obj.rol == 'RKO':
    ...
    def get_queryset(self):
        ...
        if self.rol_nu == Rol.ROL_HWL:
            ...
            objs = (Functie.objects
                    .filter(rol__in=('BKO', 'RKO', 'RCL', 'MWZ', 'SUP', 'CS'))
                    .select_related('rayon',
                                    'regio',
                                    'regio__rayon')
                    .prefetch_related('accounts'))
