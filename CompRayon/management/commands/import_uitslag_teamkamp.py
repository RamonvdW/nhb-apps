        ...
        for team in self.teams_cache:
            if team.vereniging.ver_nr == ver_nr:
                if team.team_naam.upper() == up_naam:
                    if team_klasse is None or team.klasse == team_klasse:
    ...
    def handle(self, *args, **options):
        ...
        while nix_count < 10:
            ...
            if team_klasse is None:
                team_klasse = kamp_team.klasse
            else:
                if team_klasse != kamp_team.klasse:
                    self.stderr.write('[ERROR] Inconsistente team klasse op regel %s: %s (eerdere teams: %s)' % (row_nr, kamp_team.klasse, team_klasse))
                    continue
