...
class TestRecordsCliDownload(E2EHelpers, TestCase):
    """ unittests voor de Records applicatie, management command download_records """

    cli_download = 'download_records'
    ...
    def test_download(self):
        ...
        with patch('httplib2.Http', return_value=my_service):
            f1, f2 = self.run_management_command(self.cli_download, fname,
                                                 report_exit_code=False)
            self.assertTrue("[INFO] Nationale Records downloader service" in f2.getvalue())
            self.assertTrue("[INFO] Saving to '/tmp/" in f2.getvalue())
