...
class TestCompetitie(TestCase):
    ...
    def test_beheer_favorieten_normaal(self):
        # niet-beheerder mag favorieten niet te wijzigen
...
class TestCompetitieKoppelBeheerders(TestCase):
    ...
    def test_koppelbeheerder_competitie_anon(self):
        self.client.logout()
    ...
    def test_koppelbeheerder_competitie_bb(self):
        # login als BB
        ...
        # haal de lijst met gekoppelde beheerders op voor de competitie
        competitie = Competitie.objects.all()[0]
        resp = self.client.get('/competitie/toon-beheerders/%s/' % competitie.pk)
        self.assertEqual(resp.status_code, 200)     # 200 = OK
        assert_html_ok(self, resp)
        assert_template_used(self, resp, ('competitie/koppel-beheerders-overzicht.dtl', 'plein/site_layout.dtl'))
        ...
        for deelcomp in DeelCompetitie.objects.filter(competitie=competitie):
            url = '/competitie/kies-beheerders/%s/' % deelcomp.pk
    ...
    def test_koppelbeheerder_competitie_bko(self):
        ...
        resp = self.client.get('/competitie/toon-beheerders/9999/')
        self.assertEqual(resp.status_code, 404)     # 404 = Not found
        ...
        # haal de lijst met gekoppelde beheerders op voor de competitie
        resp = self.client.get('/competitie/kies-beheerders/%s/' % de_deelcomp.pk)
        self.assertEqual(resp.status_code, 200)
        assert_html_ok(self, resp)
        assert_template_used(self, resp, ('competitie/koppel-beheerders-wijzig.dtl', 'plein/site_layout.dtl'))

        # koppel de nieuwe beheerders voor deze regio
        post_params = {
            'deelcomp_pk': de_deelcomp.pk,
            'beheerder_%s' % bko_favs[0].favoriet.pk: "on",
            'beheerder_%s' % bko_favs[1].favoriet.pk: "on",
            'beheerder_%s' % bko_favs[3].favoriet.pk: "on",
        }
        resp = self.client.post('/competitie/wijzig-beheerders/', post_params)
        self.assertEqual(resp.status_code, 302)     # 302 = Redirect
        self.assertEqual(resp.url, '/competitie/toon-beheerders/%s/' % de_deelcomp.competitie.pk)
        self.assertEqual(len(functie.user_set.all()), 3)

    def test_koppelbeheerder_competitie_rko(self):
        ...
        # koppel de nieuwe beheerder voor deze regio
        post_params = {
            'deelcomp_pk': de_deelcomp.pk,
            'beheerder_%s' % rko2_favs[0].favoriet.pk: "on",
        }
        resp = self.client.post('/competitie/wijzig-beheerders/', post_params)
        self.assertEqual(resp.status_code, 302)     # 302 = Redirect
        self.assertEqual(resp.url, '/competitie/toon-beheerders/%s/' % de_deelcomp.competitie.pk)
    ...
    def test_koppelbeheerder_competitie_rcl(self):
        ...
        resp = self.client.get('/competitie/toon-beheerders/%s/' % competitie.pk, follow=True)
        self.assertEqual(resp.status_code, 200)     # 200 = OK
        assert_html_ok(self, resp)
        assert_template_used(self, resp, ('competitie/koppel-beheerders-overzicht.dtl', 'plein/site_layout.dtl'))
