...
class Command(BaseCommand):
            ...
            if lid_nr_str:
                ...
                try:
                    ...
                    try:
                        ...
                        self.deelnemers[lid_nr] = [deelnemer]
                        self.bk_deelnemers.append(deelnemer)
            else:
                nix_count += 1
        # while
    ...
    def _importeer_finales(self, ws, aantal_finalisten):
        ...
        if self.verbose:
            self.stdout.write('[DEBUG] Aantal BK deelnemers: %s' % len(self.bk_deelnemers))

        if goud == 0:
            self.stderr.write('[ERROR] Kan winnaar van gouden finale niet vaststellen')
            return

        if brons == 0 and len(self.bk_deelnemers) > 2:
    ...
    def handle(self, *args, **options):
        ...
        self.comp = Competitie.objects.filter(afstand='18').order_by('begin_jaar')[0]    # pak de oudste
